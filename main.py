...
def generar_pdf_facturas(ruta_destino, num_factura=None, fecha_consulta=None):
    ...
    for (f_id,) in facturas:
        ...
        # Título XIOMI en azul vibrante clarito
        ...
        # Recuadro FACTURA en azul clarito con texto NEGRO
        ...
        # Encabezados de tabla en azul clarito
        ...
        # Pie de página en azul vibrante clarito
...
def generar_reporte_generico(ruta_destino, query, parametros, titulo, subtitulo):
    ...
    # Título del reporte en azul vibrante clarito
...
def main(page: ft.Page):
    ...
    try: inicializar_base_datos()
        ...
        page.add(ft.Text(f"ERROR FATAL:\n{e}", color="red", weight="bold")); return 
    ...
    def cambiar_pantalla(e):
        ...
        # PANTALLA 2: PEDIDOS DEL DÍA
            ...
            tabla_pedidos = ft.DataTable(columns=[ft.DataColumn(ft.Text(c, weight="bold")) for c in ["Factura", "Cliente", "Total", "Acciones"]], rows=[])
                    ...
                    if filas:
                        for id_f, cli, tot in filas:
                            def borrar_p(e, num=id_f):
                                ...
                                ref_pedidos()
                                page.snack_bar = ft.SnackBar(ft.Text("🗑️ Pedido anulado y borrado de la caja", color="white"), bgcolor="red")
                                page.snack_bar.open = True; page.update()

                            def editar_p(e, num=id_f):
                                estado_carrito["editando"] = True
                                estado_carrito["id_factura"] = num
                                estado_carrito["cargado"] = False
                                carrito.clear()
                                page.go("/carrito")
                            ...
                            tabla_pedidos.rows.append(ft.DataRow(cells=[
                                ...
                                ft.DataCell(ft.Text(f"${formato_ars(tot)}")), ft.DataCell(fila_botones)
                            ]))
                    tabla_pedidos.update()
                except Exception as ex: print(ex)

            def ir_nuevo_pedido(e):
                estado_carrito["editando"] = False; estado_carrito["id_factura"] = None; estado_carrito["cargado"] = False
        ...
        # PANTALLA 3: CARRITO Y NUEVO CLIENTE INSTANTÁNEO
            ...
            # --- LA MAGIA DEL CLIENTE AL INSTANTE ---
            campo_nom_cli = ft.TextField(label="Nombre y Apellido", width=300)
            campo_dir_cli = ft.TextField(label="Dirección", width=300)
            def guardar_cliente_rapido(e):
                ...
                # Esto lo inyecta en el buscador y lo selecciona solo
                dd_clientes.options.append(ft.dropdown.Option(text=nom_nuevo))
                dd_clientes.value = nom_nuevo
                dd_clientes.update()
                
                campo_nom_cli.value = ""; campo_dir_cli.value = ""; page.dialog.open = False
                page.snack_bar = ft.SnackBar(ft.Text("✅ Cliente guardado y seleccionado automáticamente", color="white"), bgcolor="green")
                page.snack_bar.open = True; page.update()
                
            page.dialog = ft.AlertDialog(title=ft.Text("Agregar Cliente Rápido"), content=ft.Column([campo_nom_cli, campo_dir_cli], tight=True), actions=[ft.TextButton("Guardar y Facturar", on_click=guardar_cliente_rapido)])
            ...
            def confirmar(e):
                ...
                if estado_carrito["editando"]:
                    ...
                    for cod, item in carrito.items():
                        ejecutar_db("INSERT INTO Detalle_Factura (numero_factura, codigo_articulo, precio_unitario, unidades, total_linea) VALUES (?, ?, ?, ?, ?)", (id_fac, cod, item["precio"], item["cantidad"], item["cantidad"] * item["precio"]))
                    page.snack_bar = ft.SnackBar(ft.Text(f"✅ Pedido Nro {id_fac} MODIFICADO y actualizado!", color="white"), bgcolor="green")
                else:
                    ...
                    for cod, item in carrito.items():
                        ejecutar_db("INSERT INTO Detalle_Factura (numero_factura, codigo_articulo, precio_unitario, unidades, total_linea) VALUES (?, ?, ?, ?, ?)", (nuevo_num, cod, item["precio"], item["cantidad"], item["cantidad"] * item["precio"]))
                    page.snack_bar = ft.SnackBar(ft.Text(f"✅ Nuevo Pedido Nro {nuevo_num} guardado en caja!", color="white"), bgcolor="green")
                
                carrito.clear(); estado_carrito["editando"] = False; page.snack_bar.open = True; page.go("/pedidos_dia")

            tit_pantalla = f"CORRIGIENDO TICKET NRO {estado_carrito['id_factura']}" if estado_carrito["editando"] else "Armar Pedido Nuevo"
            txt_boton = "GUARDAR CORRECCIÓN" if estado_carrito["editando"] else "GUARDAR VENTA"
            color_fondo = ft.colors.BLUE_800 if estado_carrito["editando"] else ft.colors.BLUE_GREY_900

            bloque = ft.Card(content=ft.Container(padding=15, content=ft.Column([
                ft.Row([dd_clientes, ft.IconButton(icon=ft.icons.PERSON_ADD, icon_color="blue", on_click=lambda _: setattr(page.dialog, 'open', True) or page.update())]), 
                ft.Text("2. Agregar o corregir artículos", weight="bold"), ft.Row([dd_productos, inp_cant]), ft.ElevatedButton("CARGAR AL TICKET", icon=ft.icons.ADD, on_click=agregar, width=float("inf"))])))
            pie = ft.Container(bgcolor=color_fondo, padding=20, border_radius=10, content=ft.Column([ft.Row([texto_total], alignment="center"), ft.ElevatedButton(txt_boton, icon=ft.icons.SAVE, on_click=confirmar, bgcolor="green", color="white", width=float("inf"), height=50)]))
        ...
        # PANTALLA 4: ABM PRODUCTOS
            ...
            tabla_prod = ft.DataTable(columns=[ft.DataColumn(ft.Text(c, weight="bold")) for c in ["Código", "Descripción", "Precio", "Eliminar"]], rows=[])
            def ref_prod():
                try:
            ...
            page.views.append(ft.View("/agregar_producto", [ft.AppBar(title=ft.Text("Catálogo"), bgcolor=ft.colors.TEAL_800, color="white", leading=ft.IconButton(ft.icons.ARROW_BACK, icon_color="white", on_click=lambda _: page.go("/"))), inp_cod, inp_nom, inp_pre, ft.ElevatedButton("GUARDAR PRODUCTO", icon=ft.icons.SAVE, on_click=guardar_p, style=estilo_btn_secundario, width=float("inf")), ft.Divider(), ft.Text("Listado Actual", weight="bold"), ft.Column([tabla_prod], scroll="auto", expand=True)], padding=20)); ref_prod()

        # PANTALLA 5: ABM CLIENTES
        elif page.route == "/agregar_cliente":
            inp_nom_c = ft.TextField(label="Nombre y Apellido", width=float("inf")); inp_dir_c = ft.TextField(label="Dirección", width=float("inf"))
            tabla_cli = ft.DataTable(columns=[ft.DataColumn(ft.Text(c, weight="bold")) for c in ["ID", "Nombre", "Dirección", "Eliminar"]], rows=[])
            def ref_cli():
                try:
            ...
            page.views.append(ft.View("/agregar_cliente", [ft.AppBar(title=ft.Text("Directorio de Clientes"), bgcolor=ft.colors.INDIGO_800, color="white", leading=ft.IconButton(ft.icons.ARROW_BACK, icon_color="white", on_click=lambda _: page.go("/"))), inp_nom_c, inp_dir_c, ft.ElevatedButton("GUARDAR CLIENTE", icon=ft.icons.SAVE, on_click=guardar_c, style=estilo_btn_secundario, width=float("inf")), ft.Divider(), ft.Text("Listado Actual", weight="bold"), ft.Column([tabla_cli], scroll="auto", expand=True)], padding=20)); ref_cli()

        # PANTALLA 6: REPORTES (MÉTODO BLINDADO PARA NAVEGADOR)
            ...
            def pedir_pdf(tipo, id_f=None, f=None, finio=None, ffin=None):
                ...
                try:
                    ...
                    if exito:
                        # Le ordenamos al navegador que abra/descargue el archivo que acabamos de crear
                        page.launch_url(f"/{nombre_archivo}")
                        page.snack_bar = ft.SnackBar(ft.Text("✅ ¡PDF Generado! Abriendo pestaña...", color="white"), bgcolor="green")
                    else:
                        page.snack_bar = ft.SnackBar(ft.Text("❌ No hay facturas cargadas en esa fecha/número.", color="white"), bgcolor="red")
                except Exception as ex:
                    page.snack_bar = ft.SnackBar(ft.Text(f"❌ Error al crear el archivo: {ex}", color="white"), bgcolor="red")
                
                page.snack_bar.open = True; page.update()
            
            page.views.append(ft.View("/reportes", [
                ft.AppBar(title=ft.Text("Reportes y Balances"), bgcolor=ft.colors.AMBER_800, color="white", leading=ft.IconButton(ft.icons.ARROW_BACK, icon_color="white", on_click=lambda _: page.go("/"))),
                ...
                ft.Card(content=ft.Container(padding=20, content=ft.Column([
                    ...
                    ft.ElevatedButton("Reporte Diario", icon=ft.icons.TODAY, on_click=lambda _: pedir_pdf("reporte_dia", f=inp_fec.value)), ft.Divider(),
                    ft.Text("Consolidado por Fechas", weight="bold"), ft.Row([inp_inicio, inp_fin]),
                    ft.ElevatedButton("Generar Balance del Período", icon=ft.icons.DATE_RANGE, bgcolor=ft.colors.TEAL_700, color="white", on_click=lambda _: pedir_pdf("consolidado", finio=inp_inicio.value.strip(), ffin=inp_fin.value.strip()))])))
...
if __name__ == "__main__":
    # ASSETS_DIR="." LE DA PERMISO AL NAVEGADOR PARA DESCARGAR LOS PDF
