...
def main(page: ft.Page):
    # --- RED DE SEGURIDAD GLOBAL: atrapa CUALQUIER error de arranque, sea donde sea ---
    try:
        _main_app(page)
        ...
        page.add(
            ft.Text(
                f"ERROR FATAL AL INICIAR LA APP:\n\n{ex}\n\n{traceback.format_exc()}",
                color="red", size=13, selectable=True,
            )
        )
        page.update()


def _main_app(page: ft.Page):
    ...
    def cambiar_pantalla(e):
        ...
        # --- PANTALLA 5: REPORTES ---
            ...
            page.views.append(ft.View("/reportes", [
                ft.AppBar(title=ft.Text("Reportes y Facturación"), bgcolor=ft.colors.AMBER_800, color=ft.colors.WHITE, leading=ft.IconButton(ft.icons.ARROW_BACK, icon_color=ft.colors.WHITE, on_click=lambda _: page.go("/"))),
                tarjeta_facturas,
                tarjeta_reporte_dia,
                tarjeta_reporte_consolidado,
        ...
        try:
            ...
            page.views.append(
                ft.View(
                    "/error",
                    [
                        ft.Text("ERROR EN PANTALLA:", color="red", size=22, weight="bold"),
                        ft.Text(str(ex), color="red", size=16, weight="bold"),
                        ft.Divider(),
                        ft.Text(detalle, color="red", size=11, selectable=True),
                    ],
                    padding=20, scroll=ft.ScrollMode.AUTO,
                )
            )
