...
# --- ESCUDO DE TITANIO 1: Atrapa errores antes de que arranque la app ---
error_de_librerias = None
...
try:
    ...
    error_de_librerias = traceback.format_exc()

# ==========================================
# 3. INTERFAZ GRÁFICA PROFESIONAL Y BLINDADA
# ==========================================
def main(page: ft.Page):
    # SI HUBO ERROR AL CARGAR LIBRERÍAS, MUESTRA ESTO SIN CRASHEAR
    if error_de_librerias:
        page.clean()
        page.add(
            ft.Text("¡ERROR AL CARGAR LIBRERÍAS!", color="white", bgcolor="red", size=24, weight="bold"),
            ft.Text("¿Pusiste 'fpdf' y 'flet' en el requirements.txt de GitHub?", weight="bold"),
            ft.Text(error_de_librerias, color="red", selectable=True)
        )
        page.update()
        return

    try:
        # ACA ARRANCA EL ESCUDO PROTECTOR PRINCIPAL
        page.title = "XIOMI Distribuidora"
        page.theme_mode = ft.ThemeMode.LIGHT
        
        # COLORES Y BOTONES CON MAYÚSCULA PARA GITHUB
        ...
        def cambiar_pantalla(e):
            try:
                ...
                page.views.clear()
                # USAMOS STRINGS "red" Y "white" ACÁ PARA QUE ESTO NO CRASHEE NUNCA
                page.views.append(
                    ft.View(
                        "/error",
                        [
                            ft.Text("¡ERROR AL DIBUJAR PANTALLA!", color="white", bgcolor="red", size=24, weight="bold"),
                            ft.Text("Sacale captura a esto por favor:", color="black", weight="bold"),
        ...
        # USAMOS STRINGS "red" Y "white" ACÁ PARA QUE ESTO NO CRASHEE NUNCA
        page.add(
            ft.Text("¡CAZAMOS EL ERROR DE ARRANQUE!", color="white", bgcolor="red", size=24, weight="bold"),
